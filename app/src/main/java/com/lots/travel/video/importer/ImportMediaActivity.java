...
package com.lots.travel.video.importer;
...
import com.lots.travel.video.crop.AliyunImageCrop;
import com.lots.travel.video.crop.AliyunVideoCrop;
import com.lots.travel.video.edit.editor.EditorActivity;
import com.lots.travel.video.importer.media.GalleryDirChooser;
import com.lots.travel.video.importer.media.GalleryMediaChooser;
import com.lots.travel.video.importer.media.MediaDir;
import com.lots.travel.video.importer.media.MediaInfo;
import com.lots.travel.video.importer.media.MediaStorage;
import com.lots.travel.video.importer.media.SelectedMediaAdapter;
import com.lots.travel.video.importer.media.SelectedMediaViewHolder;
import com.lots.travel.video.importer.media.ThumbnailGenerator;

import java.util.List;

public class ImportMediaActivity extends Activity implements View.OnClickListener {
    ...
    private void init() {
        ...
        mTransCoder.setTransCallback(new Transcoder.TransCallback() {
                    ...
                    public void run() {
                        ...
                        switch (errorCode) {
                            case AliyunErrorCode.ERROR_MEDIA_NOT_SUPPORTED_AUDIO:
                                ToastUtil.showToast(ImportMediaActivity.this, R.string.aliyun_not_supported_audio);
                                break;
                            case AliyunErrorCode.ERROR_MEDIA_NOT_SUPPORTED_VIDEO:
                                ToastUtil.showToast(ImportMediaActivity.this, R.string.aliyun_video_crop_error);
                                break;
                            case AliyunErrorCode.ERROR_UNKNOWN:
                            default:
                                ToastUtil.showToast(ImportMediaActivity.this, R.string.aliyun_video_error);
                        }
                    }
                });
            ...
            public void onComplete(List<MediaInfo> resultVideos) {
                ...
                mImport = AliyunImportCreator.getImportInstance(ImportMediaActivity.this);
                ...
                if (projectJsonPath != null) {
                    ...
                    data.putExtra(EditorActivity.KEY_VIDEO_PARAM, mVideoParam);
                    data.putExtra(EditorActivity.KEY_PROJECT_JSON_PATH,projectJsonPath);
...
//                mImport.addVideo(infoCopy.filePath, 3000, AliyunDisplayMode.DEFAULT);    //导入器中添加视频
        ...
        mSelectedVideoAdapter = new SelectedMediaAdapter(new MediaImageLoader(this), 30 * 1000);//最大时长30秒
        ...
        mSelectedVideoAdapter.setItemViewCallback(new SelectedMediaAdapter.OnItemViewCallback() {
            ...
            public void onItemPhotoClick(MediaInfo info, int position) {
                ...
                if (info.mimeType.startsWith("video")) {
                    Intent intent = new Intent(ImportMediaActivity.this, AliyunVideoCrop.class);
                    ...
                    Intent intent = new Intent(ImportMediaActivity.this, AliyunImageCrop.class);
                    intent.putExtra(CropKey.VIDEO_PATH, info.filePath);
                    intent.putExtra(CropKey.VIDEO_DURATION, info.duration);
                    intent.putExtra(CropKey.VIDEO_RATIO, mRatio);
                    intent.putExtra(CropKey.VIDEO_SCALE, scaleMode);
    ...
    public void onClick(View v) {
        if (v == back) {
            ...
            if (mIsReachedMaxDuration) {
                ToastUtil.showToast(ImportMediaActivity.this, R.string.message_max_duration_import);
                return;
            }
            //对于大于720P的视频需要走转码流程
