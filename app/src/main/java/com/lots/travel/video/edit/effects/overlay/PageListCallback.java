...
public abstract class PageListCallback implements PageRecyclerView.CallBack<PageViewHolder> {
    ...
    public void onBindViewHolder(final PageViewHolder holder, int position) {
        if (mListData != null
            ...
            Glide.with(mContext).load(model.getIcon()).into(new ViewTarget<CircularImageView,Drawable>(holder.mImageView) {
                @Override
                public void onResourceReady(Drawable resource, Transition<? super Drawable> transition) {
                    holder.mImageView.setImageBitmap(((BitmapDrawable) resource).getBitmap());
                }
            });
        }
    }
