package com.lots.travel.widget.picker.core;
...
//            throw new RuntimeException("DataSet cannot be changed while scrolling");
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(!TextUtils.isEmpty(mLabelText)) {
            String centerText = mAdapter.getText(mAnchorInfo.position);
            float centerTextWidth = mHighlightTextPaint.measureText(centerText);
            canvas.drawText(mLabelText,centerTextWidth/2+mLabelMarginLeft,getTextBaseline(mLabelPaint,0),mLabelPaint);
        }
    ...
    private void drawItem(Canvas canvas,Paint paint,int position,float y,float scaleY){
        ...
        Drawable icon = position>=0 && position<mCount ? mAdapter.getIcon(position):null;
        if(icon!=null){
