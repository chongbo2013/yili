...
public abstract class PreviewScheduleHolder extends ScheduleHolder implements TripPartDisplay.OnControlListener{

    public PreviewScheduleHolder(ScheduleAdapter adapter, View v) {
        super(adapter, v);
    }
    ...
    public void onPlayVoiceStart() {
        ...
        if(tripPart!=null && ((TypeUtil.str2int(tripPart.getTripSoundLen()))!=0) && !TextUtils.isEmpty(tripPart.getTripSound())){
            AudioPlayer audioPlayer = ((PreviewScheduleAdapter)mAdapter).getAudioPlayer();
            if(audioPlayer!=null)
