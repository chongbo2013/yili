...
public class SwipeItemLayout extends ViewGroup {
    ...
    private boolean triggerSwipeListener(MotionEvent ev){
        if(onSwipeListener!=null)
            return onSwipeListener.onSwipe(ev);
        return false;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(
                mainItemView.getMeasuredWidth()+ getPaddingLeft()+
                        getPaddingRight()+ lp.leftMargin+lp.rightMargin
                ,mainItemView.getMeasuredHeight()+getPaddingTop()+
        ...
        for(int i=0;i<getChildCount();i++){
            ...
            menuHeightSpec = MeasureSpec.makeMeasureSpec(
    ...
    private class ScrollRunnable implements Runnable{
        ...
        void startScroll(int startX,int endX){
            if(startX!=endX){
                ...
                scroller.startScroll(startX,0,endX-startX,0, 200);
    ...
    //判断为侧滑时调用改方法，如果返回true，就不处理该事件
    public interface OnSwipeListener {
        boolean onSwipe(MotionEvent ev);
    }
                ...
                case MotionEvent.ACTION_MOVE: {
                    ...
                    if(mode==Mode.CLICK){
                        ...
                        if(xDiff> touchSlop && xDiff>yDiff){
                            if(captureItem.triggerSwipeListener(ev)) {
                                ignoreActions = true;
                                return false;
                            }
