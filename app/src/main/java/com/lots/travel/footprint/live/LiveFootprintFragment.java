...
public class LiveFootprintFragment extends PagedItemFragment<Footprint> implements OnFootprintListener {
    private static final int REQ_COMMENT = 1;
    public static final int REQ_NOTE_DETAIL = 2;

    private String mEventStyle;
    private LiveFootprintAdapter mFootprintAdapter;
    private int mTargetPosition = -1;

    public void search(String eventStyle){
        mEventStyle = eventStyle;
        forceRefresh();
    }
    ...
    public PageIterator.PageRequest<Footprint> createPageRequest() {
        return new PageIterator.PageRequest<Footprint>() {
            ...
            public Single<HttpResult<List<Footprint>>> execute(int page) {
                ...
                params.setQueryType("public");
                if(mEventStyle!=null)
                    params.setEventStyle(mEventStyle);
    ...
    public AbstractLoadAdapter<Footprint> createAdapter(RecyclerView rv) {
        mFootprintAdapter = new LiveFootprintAdapter(getContext(),rv);
...
//        switch (eventStyle){
//            case GetComments.DATA_TYPE_FOOTPRINT:
//                AddFootprintCommentActivity.toComment(this,REQ_COMMENT,data);
//                break;
//        }
    }
